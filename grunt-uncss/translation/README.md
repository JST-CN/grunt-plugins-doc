...
像[Bootstrap](http://getbootstrap.com), [TopCoat](http://topcoat.io) 等等这类用于界面库相当多，然而许多开发者都只会适应它们所提供的少于10%的CSS（大部分情况下，都不会完全使用到）。 其结果可想而知，它们会以生成大量冗余的样式而告终，这会显著增加页面的加载时间并且影响你的心情。
